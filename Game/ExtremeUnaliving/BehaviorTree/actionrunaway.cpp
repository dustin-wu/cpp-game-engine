...
ActionRunAway::ActionRunAway(std::shared_ptr<GameObject> entity, std::shared_ptr<GameObject> target, std::shared_ptr<NavMeshSystem> navSystem, std::shared_ptr<AnimatedModel> animModel, std::shared_ptr<Animation> animation)
{
    m_entity = entity;
    m_target = target;
...
Status ActionRunAway::act(float deltaTime) {
     m_animModel->doAnimation(m_animation);

    if (findPathCooldown <= 0) {
        chaserPath = m_navSystem->findPathAwayFrom(m_entity->ID(), m_target->ID());
        pathProgress = 0;
